class Reporting::ThreatIntelligenceReport
  ...
  def generate(from = Time.zone.today - 14.days, to = Time.zone.today)
    messages = [
  ...
  def self.generate(from = Time.now - 14.days, to = Time.now)
    new.generate(from, to)
  end

  def self.call(from = Time.now - 14.days, to = Time.now)
    new.generate(from, to)
  end
  ...
  def prompt(from, to)
    ...
    hosts = Host.where("created_at >= ? AND created_at <= ?", from, to).limit(700)
    hosts_data = "Hosts:\n"
    hosts_data += hosts.map(&:as_vector).join("\n")

    threat_actors = ThreatActor.where("created_at >= ? AND created_at <= ?", from, to).limit(700)
    threat_actors_data = "\nThreat Actors:\n"
    ...
    predictions = Prediction.where("created_at >= ? AND created_at <= ?", from, to).limit(700)
    predictions_data = "\nPredictions:\n"
    predictions_data += predictions.map(&:as_vector).join("\n")

    malware = Malware.where("created_at >= ? AND created_at <= ?", from, to).limit(700)
    malware_data = "\nMalware:\n"
